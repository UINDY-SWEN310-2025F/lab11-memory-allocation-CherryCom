...
void firstFit(int nSeg, int nProc, int segSize[], int segUsed[], int segStart[], AllocationInfo alloc[]) {
    ...
    for(int p=0; p<nProc; p++){
        int procSize = alloc[p].size;

        for(int s=0; s<nSeg; s++){
            if(!occupied[s] && segSize[s] >= procSize){
                alloc[p].partition = s;
                alloc[p].start = segStart[s];
                alloc[p].end = segStart[s] + procSize;
                occupied[s] = 1;
                segUsed[s] = procSize;
                break;
            }
...
void bestFit(int nSeg, int nProc, int segSize[], int segUsed[], int segStart[], AllocationInfo alloc[]) {
    ...
    for(int p=0; p<nProc; p++){
        ...
        if(bestSeg != -1){
            alloc[p].partition = bestSeg;
            alloc[p].start = segStart[bestSeg];
            alloc[p].end = segStart[bestSeg] + procSize;
            occupied[bestSeg] = 1;
            segUsed[bestSeg] = procSize;
        }
    }
